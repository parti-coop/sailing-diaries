class Item < ApplicationRecord
  ...
  def self.count_emotions(month, emotion_title)
    Emotion.where(title: emotion_title).where(item_id: Item.by_month(month, year: 2017)).count
  end

  def self.count_king_emotions(month)
    Emotion.where(item_id: Item.by_month(month, year: 2017)).group(:title).count.values.max
  end

  def self.count_month(month)
    Emotion.where(item_id: Item.by_month(month, year: 2017)).count
  end
